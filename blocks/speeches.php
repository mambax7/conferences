 ...
 * @return array
 */
function showConferencesSpeeches($options)
{
    // require dirname(__DIR__) . '/class/speeches.php';
    ///  $moduleDirName = basename(dirname(__DIR__));
    //$myts = \MyTextSanitizer::getInstance();

    $block         = [];
    $blockType     = $options[0];
    $speechesCount = $options[1];
 ...
 * @return string
 */
function editConferencesSpeeches($options)
{
    //require dirname(__DIR__) . '/class/speeches.php';
    // $moduleDirName = basename(dirname(__DIR__));
    ...
    foreach (array_keys($speechesArray) as $i) {
        $id   = $speechesArray[$i]->getVar('id');
        $form .= "<option value='" . $id . "' " . (false === in_array($id, $options) ? '' : "selected='selected'") . '>' . $speechesArray[$i]->getVar('title') . '</option>';
    }
    $form .= '</select>';
