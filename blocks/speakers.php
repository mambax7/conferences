 ...
 * @return array
 */
function showConferencesSpeakers($options)
{
    // require dirname(__DIR__) . '/class/speakers.php';
    ///  $moduleDirName = basename(dirname(__DIR__));
    //$myts = \MyTextSanitizer::getInstance();

    $block         = [];
    $blockType     = $options[0];
    $speakersCount = $options[1];
    ...
    $criteria->setLimit($speakersCount);
 ...
 * @return string
 */
function editConferencesSpeakers($options)
{
    //require dirname(__DIR__) . '/class/speakers.php';
    // $moduleDirName = basename(dirname(__DIR__));
    ...
    foreach (array_keys($speakersArray) as $i) {
        $id   = $speakersArray[$i]->getVar('id');
        $form .= "<option value='" . $id . "' " . (false === in_array($id, $options) ? '' : "selected='selected'") . '>' . $speakersArray[$i]->getVar('name') . '</option>';
    }
    $form .= '</select>';
