 ...
 * @return array
 */
function showConferencesTracks($options)
{
    // require dirname(__DIR__) . '/class/tracks.php';
    ///  $moduleDirName = basename(dirname(__DIR__));
    //$myts = \MyTextSanitizer::getInstance();

    $block       = [];
    $blockType   = $options[0];
    $tracksCount = $options[1];
    ...
    $criteria->setLimit($tracksCount);
    $tracksArray = $tracksHandler->getAll($criteria);
    foreach (array_keys($tracksArray) as $i) {
        $block[$i]['cid']   = $tracksArray[$i]->getVar('cid');
        $block[$i]['title'] = $tracksArray[$i]->getVar('title');
    }
 ...
 * @return string
 */
function editConferencesTracks($options)
{
    //require dirname(__DIR__) . '/class/tracks.php';
    // $moduleDirName = basename(dirname(__DIR__));
    ...
    foreach (array_keys($tracksArray) as $i) {
        $id   = $tracksArray[$i]->getVar('id');
        $form .= "<option value='" . $id . "' " . (false === in_array($id, $options) ? '' : "selected='selected'") . '>' . $tracksArray[$i]->getVar('title') . '</option>';
    }
    $form .= '</select>';
