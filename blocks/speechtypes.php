 ...
 * @return array
 */
function showConferencesSpeechtypes($options)
{
    // require dirname(__DIR__) . '/class/speechtypes.php';
    ///  $moduleDirName = basename(dirname(__DIR__));
    //$myts = \MyTextSanitizer::getInstance();

    $block            = [];
    $blockType        = $options[0];
    $speechtypesCount = $options[1];
 ...
 * @return string
 */
function editConferencesSpeechtypes($options)
{
    //require dirname(__DIR__) . '/class/speechtypes.php';
    // $moduleDirName = basename(dirname(__DIR__));
    ...
    foreach (array_keys($speechtypesArray) as $i) {
        $id   = $speechtypesArray[$i]->getVar('id');
        $form .= "<option value='" . $id . "' " . (false === in_array($id, $options) ? '' : "selected='selected'") . '>' . $speechtypesArray[$i]->getVar('name') . '</option>';
    }
    $form .= '</select>';
